  ...
  view(){
    ...
    Router.go(`
      ...
      <table class="table is-fullwidth is-striped">
        <thead>
          <tr><th>Học viên</th><th class="has-text-centered">Số bài</th><th></th></tr>
        </thead>
        <tbody>
          ${Object.keys(data).map(u=>`
            <tr>
              ...
              <td><button class="button is-small is-info"
                          onclick="Manager.viewSub('${u}')">Xem bài</button></td>
            </tr>`).join('')}
        </tbody>
      </table>`);
  },
  ...
  viewSub(user){
    ...
    Router.go(`
      ...
      <table class="table is-fullwidth is-striped mt-4">
        <thead><tr><th>#</th><th>Tiêu đề</th><th>File</th><th>Thời gian</th></tr></thead>
        <tbody>
          ${list.map((s,i)=>`
            <tr>
              <td>${i+1}</td>
